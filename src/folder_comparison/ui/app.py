...
from folder_comparison import FolderComparator
from folder_comparison.ui.styles.colors import COLORS
...
def copiar_archivos_nuevos_ui(entry_base, entry_comparar, entry_destino, tree, progreso, root):
    ...
    comparador = FolderComparator(base, comparar, destino)
...
def construir_ui(root):
    ...
    entry_base = crear_zona_drag_drop(
        frame_drag, "📁 Carpeta Base", "Arrastra aquí o haz clic para seleccionar"
    )
    entry_comparar = crear_zona_drag_drop(
        frame_drag, "📂 Carpeta a Comparar", "Arrastra aquí o haz clic para seleccionar"
    )

    # Carpeta destino
    frame_destino = Frame(main_container, bg=COLORS["background"])
    frame_destino.pack(fill="x", pady=(0, 20))
    ttk.Label(frame_destino, text="📥 Carpeta de Destino", style="Title.TLabel").pack(
        anchor="w", pady=(0, 8)
    )
    destino_input_frame = Frame(
        frame_destino, bg=COLORS["card"], bd=1, relief="solid", highlightthickness=0
    )
    destino_input_frame.pack(fill="x")
    entry_destino = ttk.Entry(
        destino_input_frame, font=("Segoe UI", 9), state="readonly", style="Custom.TEntry"
    )
    ...
    ttk.Label(progreso_frame, text="Progreso", style="Subtitle.TLabel").pack(
        anchor="w", pady=(0, 5)
    )
    ...
    ttk.Label(tree_frame, text="Archivos Procesados", style="Subtitle.TLabel").pack(
        anchor="w", pady=(0, 5)
    )
    ...
    tree = ttk.Treeview(
        tree_container, yscrollcommand=scroll.set, selectmode="browse", show="tree headings"
    )
    ...
    ttk.Button(
        ...
        command=lambda: copiar_archivos_nuevos_ui(
            entry_base, entry_comparar, entry_destino, tree, progreso, root
        ),
